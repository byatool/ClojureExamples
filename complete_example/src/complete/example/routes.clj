(ns complete.example.routes
  (:use
   ...
   [complete.example.communicator.user-communicator :only (login-user)]
...
(defroutes main-routes
  ...
  ;; ;;curl -X POST -H "Content-Type: application/json" "http://localhost:3000/test/sean?password=hihi"
  ;; (|-| test username ?password
  ;;      (let [message-item (MessageItem. "this is the message")]
  ;;        (let [result (MessageResult. [message-item] false "redirect goes here" "")
  ;;              test-me-one (struct-map test-me :name "hi" :age 2)]
  ;;          (json-response {"item:" test-me-one}))))
